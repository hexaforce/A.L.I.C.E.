...
public class AB {
	...
	public void skipCategory(Category c) {
		 ...
		 * System.out.println(bot.unfinishedGraph.getCategories().size() +
	...
	void findPatterns(Nodemapper node, String partialPatternThatTopic) {
		if (NodemapperOperator.isLeaf(node)) {
			// System.out.println("LEAF: "+node.category.getActivationCnt()+".
			...
			if (node.category.getActivationCnt() > MagicNumbers.node_activation_cnt) {
				...
				try {
					...
					// if (brain.existsCategory(c)) System.out.println(c.inputThatTopic()+"
					// Exists");
		...
		if (NodemapperOperator.size(node) > MagicNumbers.node_size) {
			// System.out.println("STAR: "+NodemapperOperator.size(node)+".
					...
					for (int i = 0; i < sentences.length; i++) {
						...
						if (sentence.length() > 0) {
							...
							if (match == null) {
								...
								// System.out.println(count+". "+sentence+" matched
	...
	public void terminalInteraction() {
		...
		// System.out.println(filteredAtomicCategories.size()+" filtered suggested
		// categories");
