...
public class AB {
	...
	public void productivity(int runCompletedCnt, Timer timer) {
		...
		log.info("Completed " + runCompletedCnt + " in " + time + " min. Productivity " + (float) runCompletedCnt / time + " cat/min");
	}

	public void readDeletedIFCategories() {
		bot.readCertainIFCategories(deletedGraph, MagicStrings.deleted_aiml_file);
		if (MagicBooleans.trace_mode)
			log.info("--- DELETED CATEGORIES -- read " + deletedGraph.getCategories().size() + " deleted categories");
	}
	...
	public void writeDeletedIFCategories() {
		...
		log.info("--- DELETED CATEGORIES -- write " + deletedGraph.getCategories().size() + " deleted categories");
	...
	public void deleteCategory(Category c) {
		...
		log.info("--- bot.writeDeletedIFCategories()");
	...
	public void skipCategory(Category c) {
		 ...
		 * log.info(bot.unfinishedGraph.getCategories().size() +
		 * " unfinished categories"); bot.writeUnfinishedIFCategories();
		 */
	}
	...
	/**
	 * read sample inputs from filename, turn them into Paths, and add them to the
	 * graph.
	 *
	...
	void findPatterns(Nodemapper node, String partialPatternThatTopic) {
		if (NodemapperOperator.isLeaf(node)) {
			...
			if (node.category.getActivationCnt() > MagicNumbers.node_activation_cnt) {
				// log.info("LEAF: "+node.category.getActivationCnt()+".
				// "+partialPatternThatTopic+" "+node.shortCut); //Start writing to the output
				// stream
				leafPatternCnt++;
				try {
	...
	public void classifyInputs(String filename) {
		try {
				...
				if (strLine != null) {
					...
					for (int i = 0; i < sentences.length; i++) {
						...
						if (sentence.length() > 0) {
							...
							if (match == null) {
								log.info(sentence + " null match");
							} else {
								match.category.incrementActivationCnt();
								// log.info(count+". "+sentence+" matched
								// "+match.category.inputThatTopic());
							}
							count += 1;
							if (count % 10000 == 0)
			...
			log.info("Finished classifying " + count + " inputs");
			// Close the input stream
			br.close();
		} catch (Exception e) {// Catch exception if any
			log.error("Error: " + e.getMessage());
	...
	/**
	 * magically suggests new patterns for a bot. Reads an input file of sample data
	 * called logFile. Builds a graph of all the inputs. Finds new patterns in the
	 * graph that are not already in the bot. Classifies input log into those new
	 * patterns.
	 *
	 *
	 *
	...
	public void ab() {
		...
		findPatterns();
		log.info(suggestedCategories.size() + " suggested categories");
	...
	public ArrayList<Category> nonZeroActivationCount(ArrayList<Category> suggestedCategories) {
		...
		for (Category c : suggestedCategories) {
			if (c.getActivationCnt() > 0)
				result.add(c);
			// else log.info("["+c.getActivationCnt()+"] "+c.inputThatTopic());
		}
	...
	public void terminalInteraction() {
		...
		for (Category c : browserCategories) {
			try {
				ArrayList<String> samples = new ArrayList<String>(c.getMatches(bot));
	...
	public void terminalInteractionStep(Bot bot, String request, String textLine, Category c, String alicetemplate) {
		...
		if (textLine.equals("q"))
			...
			bot.writeQuit();
			/*
			 * Nodemapper udcNode = bot.brain.findNode("*", "*", "*"); if (udcNode != null)
			 * { AIMLSet udcMatches = new AIMLSet("udcmatches");
			 * udcMatches.addAll(udcNode.category.getMatches()); udcMatches.writeAIMLSet();
			 * }
			 */
			/*
			 * Nodemapper cNode = bot.brain.match("JOE MAKES BEER", "unknown", "unknown");
			 * if (cNode != null) { AIMLSet cMatches = new AIMLSet("cmatches");
