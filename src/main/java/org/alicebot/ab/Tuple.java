...
public class Tuple extends HashMap<String, String> {
	...
	public boolean equals(Object o) {
		 ...
		 * System.out.println("unequal 1"); return false; } if (!super.equals(o)) {
		 * System.out.println("unequal 2"); return false; }
		 */
		Tuple tuple = (Tuple) o;

		// if (!visibleVars.equals(tuple.visibleVars)) return false;
		if (visibleVars.size() != tuple.visibleVars.size()) {
			// System.out.println("Tuple: "+name+"!="+tuple.name+" because size
			// "+visibleVars.size()+"!="+tuple.visibleVars.size());
			return false;
		}
		// System.out.println("Tuple visibleVars = "+visibleVars+" tuple.visibleVars =
		// "+tuple.visibleVars);
		for (String x : visibleVars) {
			// System.out.println("Tuple:
			// get("+x+")="+get(x)+"tuple.get("+x+")="+tuple.get(x));
			if (!tuple.visibleVars.contains(x)) {
				// System.out.println("Tuple: "+name+"!="+tuple.name+" because
